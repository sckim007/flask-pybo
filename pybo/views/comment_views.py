...
def create_question(question_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        return redirect('{}#comment_{}'.format(url_for(
            'question.detail', question_id=question_id), comment.id))
...
def modify_question(comment_id):
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            form.populate_obj(obj=comment)
            comment.modify_date=datetime.now()
            db.session.commit()
            return redirect('{}#comment_{}'.format(url_for(
                'question.detail', question_id=comment.question_id), comment.id))
    else:
...
def create_answer(answer_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.commit()
        return redirect('{}#comment_{}'.format(url_for(
            'question.detail', question_id=answer.question_id), comment.id))
    return render_template('comment/comment_form.html', form=form)
